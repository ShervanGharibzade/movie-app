  ...
  return (
    <div className="text-white flex flex-col w-2/3 mx-auto text-center justify-center items-center">
      ...
      <Image
        ...
        alt={movie.title}
      />
      <h2 className="font-bold text-4xl py-10">{movie.original_title}</h2>
      <h3 className="text-zinc-400 text-lg py-5">
        Genre: {movie.genres[1].name} ,
        <span>Popularity: {String(movie.vote_average).slice(0, 4)}</span>
      </h3>
      <p>{movie.overview}</p>
    </div>
  );
